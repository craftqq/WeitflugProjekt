...
public class Tastatur {
    ...
    ITastatur   link        = null;
    String      rueckgabe;
    HashMap<String, Boolean> keyPressedHashmap; 
    ...
    public Tastatur() {
        ...
        keyPressedHashmap = new HashMap<String, Boolean>();
    }
    ...
    public void meldeAnTaste(char c, String rueckgabe) {
        if (ziel != null) {
            // Taste in Hashmap eintragen
            if(!keyPressedHashmap.containsKey(rueckgabe)) keyPressedHashmap.put(rueckgabe, false);
            // Key pressed
            ...
            int keyCode=1;
			KeyStroke kStr = KeyStroke.getKeyStroke(keyCode,0,true); //KeyStroke für das Zeichen c als released Event
            ziel.getInputMap(condition).put(kStr,"released "+rueckgabe);

            ziel.getActionMap().put("released "+rueckgabe, new JTaAction(this, "released "+rueckgabe));
        }
    }
    ...
    public void meldeAnTaste(String s, String rueckgabe) {
        if (ziel != null) {
            ...
            // Key released
            ziel.getInputMap(condition).put(KeyStroke.getKeyStroke("released "+s),
                "released "+s);
            ziel.getActionMap().put("released "+s, new JTaAction(this, "released "+s));
        }
    }
    ...
    /**
     * Liefert den Status der Taste zurück, die bereits registriert worden sein muss
     * Falls die Taste nicht registriert wurde wird false zurückgeliefert
     */
    public boolean istgedrueckt(String taste){
        if(keyPressedHashmap.containsKey(taste)) return keyPressedHashmap.get(taste);
        else return false;
    }

    /**
     * Setzt den Status der Taste zurück, die bereits registriert worden sein muss
     * Falls die Taste nicht registriert wurde wird, geschieht nichts
     */
    public void setzeTasteGedrueckt(String taste, boolean value){
        if(keyPressedHashmap.containsKey(taste)) keyPressedHashmap.put(taste,value);
...
class JTaAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent arg0) {
        ...
        if (link != null) {
            ...
            if(rueckgabe.startsWith("released ")){
                // Key released event

                link.setzeTasteGedrueckt(rueckgabe.substring(9),false);
            }
            else{
                // Key pressed event

                link.setzeTasteGedrueckt(rueckgabe,true);
